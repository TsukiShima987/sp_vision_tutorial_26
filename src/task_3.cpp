...
int main(int argc, char * argv[])
{
  ...
  const int SPEED_GEARS=3;
  const int MAX_SHOTS_PER_GEAR=10;
  const double FIXED_DISTANCE=2.0;
  const double DISTANCE_TOLERANCE=0.2;
  ...
  int current_gear=0;
  int current_shots=0;
  bool is_gear_completed=false;
  bool is_waiting_gear=true;

  Eigen::VectorXd P0_dig(11);
  P0_dig << 0.01,0.01,0.01,0.01,0.01,0.01,0.1,0.1,0.01,0.01,0.01;
  ...
  while (!exiter.exit()) {
    ...
    if(is_gear_completed){
      cv::imshow("Task 3",img);
      if(cv::waitKey(1)>=0){
        current_gear++;
        current_shots=0;
        is_gear_completed=false;
        is_waiting_gear=false;
        target.reset();
        if(current_gear>=SPEED_GEARS){
          cv::imshow("Task 3",img);
          cv::waitKey(5000);
          break;
        }
    ...
    for(const auto& armor : detected_armors){
      ...
      if(std::abs(armor_distance-FIXED_DISTANCE)<=DISTANCE_TOLERANCE&&!armor.duplicated){
    ...
    auto target_armor=*std::min_element(valid_armors.begin(), valid_armors.end(),[&](const auto& a, const auto& b){
        return std::abs(a.xyz_in_gimbal.norm()-FIXED_DISTANCE)<std::abs(b.xyz_in_gimbal.norm()-FIXED_DISTANCE);
      });
    ...
    if(!target.has_value()){
      target.emplace(target_armor,t,P0_dig,0.2,4);
    } else {
      target->predict(t);
    ...
    if(current_shots<MAX_SHOTS_PER_GEAR&&!is_waiting_gear&&target->convergened()){
      ...
      if(current_shots>=MAX_SHOTS_PER_GEAR){
        is_gear_completed=true;
        gimbal.send(0,0,0.0f,0.0f);
      }
    } 
    else if(current_shots>=MAX_SHOTS_PER_GEAR&&!is_gear_completed){
      is_gear_completed=true;
      gimbal.send(0,0,0.0f,0.0f);
    }
    ...
    plot_data["gimbal_control"]["is_fire"]=(current_shots < MAX_SHOTS_PER_GEAR)?1:0;
    ...
    if(target.has_value()){
      plot_data["ekf"]["fitted_angular_velocity_rad/s"]=target->ekf_x()(7);
      plot_data["ekf"]["is_converged"]=target->convergened()?1:0;
    }
    plotter.plot(plot_data);

    std::string gear_name[]={"Low(3-5rad/s)","Mid (5-7rad/s)","High (8-10rad/s)"};
    cv::putText(img, fmt::format("Task3：{} | Shots: {}/{}", gear_name[current_gear], current_shots, MAX_SHOTS_PER_GEAR), 
    ...
    if(target.has_value()){
      cv::putText(img, fmt::format("EKF Angular Velocity：{:.2f}rad/s | Converged：{}", 
        target->ekf_x()(7), target->convergened() ? "Yes" : "No"), 
        cv::Point(20, 150), cv::FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(0, 255, 255), 2);
    }
